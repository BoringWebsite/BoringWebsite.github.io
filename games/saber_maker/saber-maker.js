class SaberMaker {
    constructor() {
        ...
        if (savedSaber) {
            ...
            this.bladeOn = savedSaber.bladeOn !== undefined ? savedSaber.bladeOn : true;
    ...
    shuffleSaber() {
        ...
        if (shuffleButton) {
            ...
            shuffleButton.style.background = 'linear-gradient(45deg, #e74c3c, #f39c12)';
            setTimeout(() => {
    ...
    startFightingGame() {
        ...
        // For now, just log the configuration (later we'll launch the fighting game)
        console.log('Starting fighting game with saber:', saberConfig);
        
        // TODO: Later we'll do something like:
        // window.location.href = 'fighting-game.html?saber=' + encodeURIComponent(JSON.stringify(saberConfig));
        // or create a new SaberFighter class
        
        alert('Fighting game coming soon! Your saber has been saved and will be used in combat.');
    }
    ...
    confirmDeleteSavedData() {
        ...
        const confirmed = confirm(
            ...
            `• Pommel: ${savedConfig.pommelType || 'Unknown'} (${savedConfig.partColors?.pommel || 'Unknown'})\n` +
            `• Grip: ${savedConfig.gripType || 'Unknown'} (${savedConfig.partColors?.grip || 'Unknown'})\n` +
            `• Emitter: ${savedConfig.emitterType || 'Unknown'} (${savedConfig.partColors?.emitter || 'Unknown'})\n\n` +
